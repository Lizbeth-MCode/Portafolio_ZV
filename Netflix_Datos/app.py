...
if archivo is not None:
    ...
    # st.session_state["dfn_copia"] = dfn_copia
    ...
    # st.success(
    #     "Proceso completo: carga, auditoría, limpieza y creación de variables realizado correctamente."
    # )
    ...
    #     Esto permite identificar problemas antes
    #     de realizar la limpieza.
    #     """
    # )

    # st.subheader("Todas las columnas del archivo")
    # st.write(dfn.columns.tolist())

    # st.subheader("Resumen numérico")
    # st.dataframe(dfn.describe())

    # st.subheader("Resumen de valores faltantes, tipo de datos y valores completos")
    ...
    #     "Nulos": ntn,
    #     "%": ntn_pct,
    #     "No nulos": dfn.notnull().sum(),
    #     "Tipo": dfn.dtypes
    # }).sort_values(by="Nulos", ascending=False)

    # st.dataframe(info)
    
    # st.header("3. Exploración inicial del contenido")

    # st.subheader("Tipos de contenido disponibles")
    ...
    # st.subheader("Años de lanzamiento existentes")
    ...
    # st.subheader("Cantidad de contenidos por año")

    # cantidad_años = dfn["release_year"].value_counts().to_frame().T

    # st.dataframe(cantidad_años)

    # st.subheader("Contenido por país")

    # paises = dfn["country"].value_counts().to_frame()

    # st.dataframe(paises)

    # st.subheader("Cantidad de categorías")

    # categorias = dfn["listed_in"].value_counts().to_frame().T

    # st.dataframe(categorias)

    # st.subheader("Clasificaciones existentes")

    # rating = dfn["rating"].value_counts().to_frame().T

    # st.dataframe(rating)

    # st.header("4. Auditoría de calidad")
    ...
    # st.subheader("Cantidad de valores diferentes por columna")

    # valores_unicos = dfn.nunique().to_frame().T

    # st.dataframe(valores_unicos)

    # st.subheader("Cantidad de directores")

    # cantidad_directores = dfn["director"].nunique()

    # st.write(cantidad_directores)

    # st.subheader("Auditoría de valores faltantes")
